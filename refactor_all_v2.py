...
# 1. Add States safely below `const [searchTerm`
...
tab_menu = """
                    ...
                    <div className="pos-tabs">
                        ...
                        <button className={`pos-tab-btn ${activeTab === 'pos' ? 'active' : ''}`} onClick={() => setActiveTab('pos')}>Point of Sale</button>
...
if "className=\"pos-tabs\"" not in content:
    ...
    # Close overview, open POS
    content = content.replace("                    {/* Main Layout Area */}", 
                              "                    </>)}\n                    {activeTab === 'pos' && (<>\n                    {/* Main Layout Area */}")
...
# 7. Inject Modals Safely exactly at the end of the return statement of BusinessView
modals_jsx = """
...
if "CUSTOM PIN PROMPT MODAL" not in content:
    # Instead of replacing global `        </div>\n    );\n}`, which matches other components,
    # Let's replace ONLY the LAST occurrence, which is the BusinessView component end.
    
    # We split by "export default BusinessView;" to ensure we only get the end of the file.
    parts = content.split('export default BusinessView;')
    if len(parts) == 2:
        parts[0] = parts[0].replace('        </div>\n    );\n}\n\n', modals_jsx + '\n        </div>\n    );\n}\n\n')
        content = parts[0] + 'export default BusinessView;'
